...
#define LDR_PIN 6           // пин куда подключен LDR
#define LDR_MIN 280         //мин яркость
#define LDR_MAX 600         //макс яркость

int max_bright = 255;          // максимальная яркость (0 - 255)
boolean adapt_light = 1;       // адаптивная подсветка (1 - включить, 0 - выключить)

byte fav_modes[] = {2,3,4,5,6,7,8,10,11,12,13,14,15,16,17,19,20,23,25,27,28,30,33,34,35,36,37,38,39,40,41};  // список "любимых" режимов
...
void loop() {
  if (adapt_light) {                        // если включена адаптивная яркость
    if (millis() - last_bright > 500) {     // каждые полсекунды
      ...
      int analog_value = analogRead(LDR_PIN);

      new_bright = map(analog_value, LDR_MIN, LDR_MAX, 3, max_bright);   // считать показания с фоторезистора, перевести диапазон
      if (new_bright > 255) {
        new_bright = 255;
      }
      if (new_bright<0) {
